<template>
  ...
  >
    ...
    <!-- 顶部刷新区域 -->
    <div v-if="topLoadMore" v-show="dargState !== 'none' && touchDistance >= 20" ref="top" class="infinite-top-container">
      <slot name="top" :dargState="dargState" :dargDistance="touchDistance">
        <div class="infinite-top-content" :style="{color:topTextColor}">
          <span v-if="dargState === 'pull' || dargState === 'drop'" class="infinite-top-content-icon icon-arrow" :class="dargState === 'drop' ? 'icon-reverse':''">
          ...
          <span v-if="dargState === 'loading'" class="infinite-top-content-icon icon-revolve">
    ...
    <div ref="content" class="infinite-list">
      <div v-for="(row,row_index) in visibleData" :id="row._key" ref="items" :key="row._key" class="infinite-list-item-container">
        <template v-for="(item,col_index) in row.value">
          <div :key="row._key + '-' + col_index" class="infinite-item">
            <slot name="default" :item="item" :row="row_index" :col="col_index"></slot>
          </div>
        </template>
        <!-- 空占位 -->
        <template v-if="row.value.length < column">
          <div v-for="(item,index) in (column - row.value.length%column)" :key="'empty-' + index" class="infinite-item">
          </div>
        </template>
      </div>
