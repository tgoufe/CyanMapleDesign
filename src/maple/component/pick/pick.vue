<template>
    ...
    >
        ...
        <div class="pos-r cmui-picker_content">
            <div class="cmui-picker__wrapper flex-container vfull overflow-h" ref="wheelWrapper">
                <div v-for="(data,index) in pickerData" :key="index" class="flex1 swiper-container">
                    <ul class="cmui-picker__scroll swiper-wrapper">
                        <li v-for="(item,index) in data" class="cmui-picker__item text-center swiper-slide text-limit1" style="height:auto" :key="index">
                            {{item.text}}
                        </li>
                    </ul>
                </div>
...
<script>
  ...
  function formatData(data){
    ...
    return _.map(data,(whleeData,index)=>formatWheelData(whleeData,_.assign(lengthStore,{tempLen:index+1}))).concat(_.fill(Array(lengthStore.maxLen-data.length),[{text:'',value:undefined}]));
  }
