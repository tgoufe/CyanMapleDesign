<template>
	<div class="cmui-captcha">
		<div class="cmui-captcha__warp flex-container center" @click="inputStart">
			<div class="cmui-captcha__item ratio-container pos-r" v-for="(item,index) in length" :class="{active:index===activeIndex}">
				<span class="flex-container center">
					<span class="cmui-captcha__dot" v-if="hide&&(index<value.length)"></span>
		...
		<form action="">
			<input :type="(type==='number')?'tel':'text'" name="" class="cmui-captcha__input" v-model="value" ref="input" @blur="inputBlur">
		</form>
	</div>
</template>
